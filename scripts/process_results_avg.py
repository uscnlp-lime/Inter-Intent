...
def process_game(eval_file_path, results_path):
    ...
    roles = list(results_data['character_to_player'].keys())
    ...
    # ### Round metrics
    metrics_form_cnt_think_rnd = eval_df_form_cnt_denorm.groupby(['player', 'side', 'intent_selection', 'intent_fol_think_1', 'round', 'round_quest_result']).size().reset_index(name='count')
    metrics_form_cnt_speech_rnd = eval_df_form_cnt_denorm.groupby(['player', 'side', 'intent_selection', 'intent_fol_speech_1', 'round', 'round_quest_result']).size().reset_index(name='count')

    metrics_ref_cnt_think_rnd = eval_df_ref_cnt_denorm.groupby(['player', 'side', 'intent_selection_final', 'intent_fol_think_2', 'round', 'round_quest_result']).size().reset_index(name='count')
    metrics_ref_cnt_speech_rnd = eval_df_ref_cnt_denorm.groupby(['player', 'side', 'intent_selection_final', 'intent_fol_speech_2', 'round', 'round_quest_result']).size().reset_index(name='count')
    ...
    metrics_form_cnt_speech_rnd['incorrect_team_sel'] = metrics_form_cnt_speech_rnd['round'].apply(lambda x: incorrect_rnds[str(x)])
    metrics_form_cnt_speech_rnd['role'] = metrics_form_cnt_speech_rnd['player'].apply(lambda x: player_to_char[x])
    ...
    metrics_form_cnt_speech_rnd['int_fol_bin'] = metrics_form_cnt_speech_rnd['intent_fol_speech'].apply(lambda x: 1 if x >= 3 else 0)
    rnd_discussion = metrics_form_cnt_speech_rnd[metrics_form_cnt_speech_rnd['int_fol_bin'] == 1].groupby(['role', 'intent_selection', 'round'])['count'].sum().reset_index(name='count')
    ...
    def round_metrics(intent_sel, rnd, role, count):
        if role not in round_metrics_dict:
            round_metrics_dict[role] = {}
        if rnd not in round_metrics_dict[role]:
            round_metrics_dict[role][rnd] = {}
        round_metrics_dict[role][rnd][intent_sel] = count


    for index, row in rnd_discussion.iterrows():
        round_metrics(row['intent_selection'], row['rnd'], row['role'], row['count'])
    ...
    for key in quest_results.keys():
        ...
        for role in roles:
            if role in evil_roles and role in round_metrics_dict:
                if int_key in round_metrics_dict[role]:
                    followed, not_followed = 0, 0
                    if 0 in round_metrics_dict[role][int_key]:
                        not_followed = round_metrics_dict[role][int_key][0]
                        
                    if 1 in round_metrics_dict[role][int_key]:
                        followed = round_metrics_dict[role][int_key][1]
                    
                    total = not_followed + followed
                    evil_per_sum += (followed/total)
                    evil_turn_count += 1
                
            else:
                if role in round_metrics_dict and int_key in round_metrics_dict[role]:
                    followed, not_followed = 0, 0
                    if 0 in round_metrics_dict[role][int_key]:
                        not_followed = round_metrics_dict[role][int_key][0]
                    ...
                    total = not_followed + followed
                    loyal_per_sum += (followed/total)
                    loyal_turn_count += 1
        
        if evil_turn_count == 0 or loyal_turn_count == 0:
            continue
        evil_per = evil_per_sum/evil_turn_count
        loyal_per = loyal_per_sum/loyal_turn_count
