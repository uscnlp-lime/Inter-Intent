...
class MistralChat(IntelligenceBackend):
    stateful = False
    type_name = "mistral-chat"
    def __init__(self, temperature: float = DEFAULT_TEMPERATURE, top_p:float=DEFAULT_TOP_P, max_tokens: int = DEFAULT_MAX_TOKENS,
        ...
        self.tokenize = AutoTokenizer.from_pretrained(self.model)
        self.tokenize.add_special_tokens({'pad_token': '[PAD]'})
