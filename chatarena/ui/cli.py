...
class ArenaCLI:
    ...
    def launch(self, max_steps: int = None, interactive: bool = False,output_file:str=''):
        """
        Run the CLI
        """
        ...
        char_to_player=self.arena.environment.char_to_name
        for char in char_to_player:
            f.write(char+':'+char_to_player[char]+'\n')
        ...
        while not timestep.terminal:
            ...
            # Print and save the new messages
            for msg in messages:
                ...
                # adding to dict for saving dataframe
                data[msg_count] = [msg.agent_name, msg.visible_to, msg.content, msg.turn, msg.timestamp, msg.msg_type]
                msg_count += 1
                console.print(message_text)
                f.write(message_text.plain)
                f.write('\n')
                msg.logged = True

            conversation_df = pd.DataFrame.from_dict(data, orient='index')
            conversation_df.to_csv(output_file + '.csv', mode='a', index=False, header=False)
            data = {}

            step += 1
            if env._terminal==True:
                break
